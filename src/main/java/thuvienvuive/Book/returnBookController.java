...
public class returnBookController implements Initializable {
    ...
    private void show(ObservableList<bookIssueDTO> List) throws Exception {
    ...
    public void handler(){
        ...
        deleteButton.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                ...
                String currentStatus = selectRow().getTrangThai();
                if(normalizeString(status).contains(normalizeString(currentStatus)) ||
                        normalizeString(currentStatus).contains(normalizeString("Đã mất"))){
                    try {
                        update(selectRow(), status, selectBookAmount(selectRow(), BookList));
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private ObservableList<bookIssueDTO> readListByStatus(ObservableList<bookIssueDTO> List) throws Exception{
    ...
    private bookIssueDTO selectRow(){
        if (issueBookList.getSelectionModel().getSelectedIndex() < 0)
            return null;
        return issueBookList.getSelectionModel().getSelectedItem();
    }
